...
def main():
    ...
    for file in os.listdir(saved_models_dir):
        if file.endswith('_baseline_model.joblib'):
            position = file.replace('_baseline_model.joblib', '')
            model_path = os.path.join(saved_models_dir, file)
            baseline_models.append((position, model_path))
    
    print(f"Found {len(baseline_models)} baseline models: {[pos for pos, _ in baseline_models]}")
    
    # Inspect each baseline model
    model_results = []
    for position, model_path in baseline_models:
        result = inspect_model_features(model_path, position, 'baseline')
