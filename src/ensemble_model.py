...
class EnsembleFantasyModel:
    ...
    def __init__(self, position: str):
        ...
        # Validate that optimized hyperparameters exist - FAIL if not found
        if not hasattr(config, 'OPTIMIZED_HYPERPARAMETERS'):
            raise ValueError("CRITICAL: No OPTIMIZED_HYPERPARAMETERS found in config")
        
        if position not in config.OPTIMIZED_HYPERPARAMETERS['random_forest']:
            raise ValueError(f"CRITICAL: No optimized RandomForest hyperparameters found for {position}")
        
        if position not in config.OPTIMIZED_HYPERPARAMETERS['lightgbm']:
            raise ValueError(f"CRITICAL: No optimized LightGBM hyperparameters found for {position}")
