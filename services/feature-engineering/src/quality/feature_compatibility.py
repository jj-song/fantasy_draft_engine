...
class FeatureCompatibilityMapper:
    ...
    def __init__(self):
        ...
        self.core_mappings = {
            ...
            'carries': 'rushing_attempts',  # CRITICAL: Handle both directions
            'games_played': 'games',
            'games': 'games_played',       # CRITICAL: Handle both directions
    ...
    def create_compatible_features(
        ...
        # Step 2: Handle special conversions
        conversion_count = 0
        
        # birth_date → age conversion
        ...
        # Handle carries ↔ rushing_attempts bidirectionally
        if 'carries' in compatible_df.columns and 'rushing_attempts' in expected_features:
            compatible_df['rushing_attempts'] = compatible_df['carries']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped carries to rushing_attempts")
        
        if 'rushing_attempts' in compatible_df.columns and 'carries' in expected_features:
            compatible_df['carries'] = compatible_df['rushing_attempts']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped rushing_attempts to carries")
        
        # Handle games ↔ games_played bidirectionally  
        if 'games' in compatible_df.columns and 'games_played' in expected_features:
            compatible_df['games_played'] = compatible_df['games']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped games to games_played")
        
        if 'games_played' in compatible_df.columns and 'games' in expected_features:
            compatible_df['games'] = compatible_df['games_played']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped games_played to games")
        
        # Handle passing attempts variations
        if 'passing_attempts' in compatible_df.columns and 'attempts' in expected_features:
            compatible_df['attempts'] = compatible_df['passing_attempts']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped passing_attempts to attempts")
        
        if 'attempts' in compatible_df.columns and 'passing_attempts' in expected_features:
            compatible_df['passing_attempts'] = compatible_df['attempts']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped attempts to passing_attempts")
        
        # Handle completions variations
        if 'passing_completions' in compatible_df.columns and 'completions' in expected_features:
            compatible_df['completions'] = compatible_df['passing_completions']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped passing_completions to completions")
        
        if 'completions' in compatible_df.columns and 'passing_completions' in expected_features:
            compatible_df['passing_completions'] = compatible_df['completions']
            conversion_count += 1
            self.logger.info(f"   ✅ Mapped completions to passing_completions")
        
        self.logger.info(f"   ✅ Applied {conversion_count} special conversions")
    ...
    def scale_predictions(self, predictions: np.ndarray, position: str, 
                         games_played: Optional[pd.Series] = None) -> np.ndarray:
        """
        Scale predictions from per-game to seasonal totals if needed.
        
        Args:
            predictions: Model predictions (potentially per-game)
            position: Player position for context
            games_played: Games played for scaling (if available)
            
        Returns:
            Scaled predictions (seasonal totals)
        """
        self.logger.info(f"🔢 Scaling {position} predictions for seasonal totals")
        
        # Check if predictions look like per-game values (too small for seasonal)
        pred_mean = np.mean(predictions)
        pred_max = np.max(predictions)
        
        # Position-specific thresholds to detect per-game vs seasonal predictions
        seasonal_thresholds = {
            'QB': {'min_mean': 200, 'min_max': 300},  # QB seasonal should be 200+ mean, 300+ max
            'RB': {'min_mean': 100, 'min_max': 200},  # RB seasonal should be 100+ mean, 200+ max  
            'WR': {'min_mean': 80, 'min_max': 150},   # WR seasonal should be 80+ mean, 150+ max
            'TE': {'min_mean': 60, 'min_max': 120}    # TE seasonal should be 60+ mean, 120+ max
        }
        
        thresholds = seasonal_thresholds.get(position, {'min_mean': 100, 'min_max': 200})
        
        # If predictions are too small, they're likely per-game - scale them up
        if pred_mean < thresholds['min_mean'] or pred_max < thresholds['min_max']:
            self.logger.info(f"   Detected per-game predictions (mean={pred_mean:.1f}, max={pred_max:.1f})")
            
            if games_played is not None:
                # Use actual games played for scaling
                scaled_predictions = predictions * games_played.values
                self.logger.info(f"   Scaled using actual games played")
            else:
                # Use position-specific default games for scaling
                default_games = {'QB': 16, 'RB': 14, 'WR': 15, 'TE': 15}
                games = default_games.get(position, 15)
                scaled_predictions = predictions * games
                self.logger.info(f"   Scaled using default {games} games for {position}")
            
            scaled_mean = np.mean(scaled_predictions)
            scaled_max = np.max(scaled_predictions)
            self.logger.info(f"   After scaling: mean={scaled_mean:.1f}, max={scaled_max:.1f}")
            
            return scaled_predictions
        else:
            self.logger.info(f"   Predictions already seasonal scale (mean={pred_mean:.1f}, max={pred_max:.1f})")
            return predictions
    
    def validate_feature_mapping(self, original_features: List[str], 
                                mapped_features: List[str], position: str) -> Dict[str, str]:
        """
        Validate and report on feature mapping success.
        
        Args:
            original_features: Original feature names from enhanced pipeline
            mapped_features: Expected feature names for legacy model
            position: Player position for context
            
        Returns:
            Dictionary with mapping validation results
        """
        self.logger.info(f"🔍 Validating feature mapping for {position}")
        
        # Calculate mapping statistics
        total_expected = len(mapped_features)
        total_original = len(original_features)
        
        # Find successful mappings
        successful_mappings = set(original_features) & set(mapped_features)
        direct_matches = len(successful_mappings)
        
        # Find features that need creation or fallback
        missing_features = set(mapped_features) - set(original_features)
        
        # Features that can be created synthetically
        creatable_features = set(missing_features) & set(self.synthetic_features.keys())
        
        # Features that will use fallback values
        fallback_features = missing_features - creatable_features
        
        # Calculate compatibility score
        compatibility_score = (direct_matches + len(creatable_features)) / total_expected
        
        # Create validation report
        validation_report = {
            'position': position,
            'total_expected_features': total_expected,
            'total_original_features': total_original,
            'direct_matches': direct_matches,
            'synthetic_features': len(creatable_features),
            'fallback_features': len(fallback_features),
            'compatibility_score': compatibility_score,
            'missing_critical': list(fallback_features)[:10],  # Show first 10 missing
        }
        
        # Log validation results
        self.logger.info(f"   📊 Mapping Results:")
        self.logger.info(f"      Direct matches: {direct_matches}/{total_expected} ({direct_matches/total_expected*100:.1f}%)")
        self.logger.info(f"      Synthetic features: {len(creatable_features)}")
        self.logger.info(f"      Fallback features: {len(fallback_features)}")
        self.logger.info(f"      Compatibility score: {compatibility_score:.3f}")
        
        if compatibility_score < 0.8:
            self.logger.warning(f"   ⚠️ Low compatibility score for {position}: {compatibility_score:.3f}")
            if fallback_features:
                self.logger.warning(f"   Missing critical features: {list(fallback_features)[:5]}")
        else:
            self.logger.info(f"   ✅ Good compatibility score for {position}")
