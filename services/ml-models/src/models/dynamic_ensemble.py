...
class DynamicEnsembleWeighter:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("DYNAMIC_ENSEMBLE_WEIGHTS not found in config, using defaults")
            self.dynamic_weights = self._get_default_dynamic_weights()
        
        try:
            self.archetype_thresholds = config.PLAYER_ARCHETYPE_THRESHOLDS
        except AttributeError:
            logger.warning("PLAYER_ARCHETYPE_THRESHOLDS not found in config, using defaults")
            self.archetype_thresholds = self._get_default_archetype_thresholds()
        
        try:
            self.default_weights = config.MODEL_WEIGHTS
        except AttributeError:
            logger.warning("MODEL_WEIGHTS not found in config, using 50/50 split")
            self.default_weights = {'random_forest': 0.5, 'lightgbm': 0.5}
    ...
    def _get_default_dynamic_weights(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        """Get default dynamic ensemble weights if config is unavailable."""
        return {
    ...
    def _get_default_archetype_thresholds(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        """Get default archetype thresholds if config is unavailable."""
        return {
